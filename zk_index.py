...
• Indexes the body content of notes in the 'body' field.
• Indexes file size and word count.
...
Command line arguments:
  ...
  --workers <int>        : Set the number of worker processes for indexing.
"""
...
DEFAULT_WORKERS = 8  # Define a default number of workers
...
def resolve_config_value(config: Dict[str, Any], key_path: str, default_value: Any) -> Any:
    """Resolves a value from the config dictionary using a key path.

    Args:
        config: The configuration dictionary.
        key_path: The path to the value in the config (e.g., "zk_index.index_file").
        default_value: The default value to return if the key path is not found.

    Returns:
        The resolved configuration value or the default value.
    """
    keys = key_path.split('.')
    current = config
    for key in keys:
...
def scandir_recursive(root: str, exclude_patterns: Optional[List[str]] = None) -> List[str]:
    """Recursively finds files under root using os.scandir, excluding paths containing any exclude patterns.
    ...
    Returns:
        A list of file paths found under the root directory, excluding specified patterns.
    """
...
def extract_frontmatter_and_body(content: str) -> Tuple[Dict[str, Any], str]:
    """Extracts YAML front matter and body from markdown content.
    ...
    Returns:
        A tuple containing:
            - A dictionary of YAML front matter (may be empty).
    ...
    if content.startswith('---'):
        ...
        if len(parts) >= 3:
            ...
            body = parts[2].strip()  # Strip body content as well
            try:
                ...
                if not isinstance(meta, dict):
                    ...
                    meta = {}  # Ensure meta is always a dict to avoid issues later
                else:
                    meta = json_ready(meta)  # Only json_ready if it's a dict
            except yaml.YAMLError as e:
                logger.warning(f"YAML parsing error: {e}. Ignoring frontmatter.")
                meta = {}
    else:
        body = body.strip()  # Strip body content if no frontmatter
    return meta, body
...
def extract_wikilinks(body: str) -> List[str]:
    ...
    Returns:
        A list of unique wikilinks found in the body.
    """
...
def calculate_word_count(body: str) -> int:
    ...
    Args:
        body: The string to count words in.
...
def process_markdown_file(filepath: str, fd_exclude_patterns: List[str], notes_dir: str) -> Optional[Dict[str, Any]]:
    """Processes a single markdown file to extract metadata, body, and links.

    Args:
        filepath: The path to the markdown file.
        fd_exclude_patterns: List of exclude patterns (for defensive check, though should be filtered earlier).
        notes_dir: The base notes directory.

    Returns:
        A dictionary containing extracted information, or None if processing fails.
    """
    # # Defensive check (should have been filtered during scanning)
    # if any(pat in filepath for pat in fd_exclude_patterns):
    #     return None
    ...
    result: Dict[str, Any] = {
        ...
        "file_size": file_size,
    }  # Added 'body', 'word_count', and 'file_size' to result
    result.update(meta)
    return result


def load_index_state(state_file: str) -> Dict[str, float]:
    """Loads the previous index state from a JSON file, handling potential errors.

    Args:
        state_file: Path to the index state file.

    Returns:
        A dictionary representing the previous index state (filepath: modification timestamp),
    ...
    if os.path.exists(state_file):
        try:
            ...
            logger.info(f"Index state file '{state_file}' not found. Starting fresh.")  # Log as info for first run
        except OSError as e:
            logger.error(f"OS error reading index state file '{state_file}': {e}. Starting fresh.")
        return {}  # Return empty dict on any failure to start fresh.
    return {}


def save_index_state(state_file: str, index_state: Dict[str, float]) -> None:
    """Saves the current index state to a JSON file, with error handling.

    Args:
        state_file: Path to the index state file.
        index_state: The index state dictionary to save.
    """
    try:
...
def load_existing_index(index_file: str) -> Dict[str, Dict[str, Any]]:
    """Loads the existing index from JSON, returning an empty dict if loading fails.

    Args:
        index_file: Path to the index file.

    Returns:
        A dictionary representing the existing index (filename: index item),
        or an empty dictionary if loading fails.
    """
...
def write_updated_index(index_file: str, updated_index_data: List[Dict[str, Any]]) -> None:
    """Writes the updated index data to the index file, exiting on failure.

    Args:
        index_file: Path to the index file.
        updated_index_data: List of index items to write.
    """
    try:
...
def remove_index_state_file(state_file: str, verbose: bool) -> None:
    """Removes the index state file, handling potential errors and logging verbose output.
...
def main() -> None:
    ...
    notes_dir_default = "/home/calluma/Dropbox/notes"  # Adjust as needed, consider making this configurable in config file too
    ...
    with ProcessPoolExecutor(max_workers=args.workers) as executor:
        ...
        for future in tqdm(as_completed(futures), total=len(futures), desc="Processing files"):  # Add tqdm here
            filepath = futures[future]  # Get the filepath back to log errors correctly
            try:
    ...
    if not args.full_reindex:
        ...
        logger.info("Skipping saving state file because of full reindex.")
