...
def run_wikilink_generator(
    ...
    # Send the processed link to the active tmux pane or clipboard
    if os.environ.get('TMUX'):
        ...
        try:
            ...
            if debug:
                logger.debug("Copied link to clipboard using pyperclip")
        except ImportError:
            logger.error("pyperclip library not found. Please install it: pip install pyperclip")
            logger.error("Falling back to printing wikilink to stdout.") # Added fallback message
            print(wikilink) # Fallback to printing to stdout
            sys.exit(1)
        except pyperclip.PyperclipException as e:
            logger.error(f"Error copying to clipboard using pyperclip: {e}")
            logger.error("Please ensure you have a clipboard mechanism installed (e.g., xclip, xsel, wl-clipboard on Linux, or that your system has clipboard support).")
            logger.error("Falling back to printing wikilink to stdout.") # Added fallback message
            print(wikilink) # Fallback to printing to stdout
            sys.exit(1)
