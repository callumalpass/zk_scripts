...
class WorkoutExercise:
    id: str
    title: str  # Save the title so it can be displayed in session summaries
...
class DataManager:
    ...
    def __init__(self, notes_dir: Path, index_file: Path, log_file: Path, backup_interval_days: int = 1):
        ...
        self._last_backup_check = '2100-01-01 00:00:00'  # Initialize to a date in the distant future
...
class UIManager:
    ...
    # --- Add to Most Recent Workout function ---
    ...
    def edit_template(self, tmpl_to_edit: Optional[WorkoutTemplate] = None) -> None:
        ...
        while True:
            ...
            if ch in (ord('a'), ord('A')):
                # Add an exercise
                ex_to_add = self.choose_exercise([])
    ...
    def _multi_select_list(self, options: List[str], title: str, initial_selection: List[int] = None) -> List[str]:
        """Helper function for a multi-select list using curses."""
        cursor = 0
        selected = initial_selection or []  # List of selected indices
        while True:
            self.clear_screen()
    ...
    def inline_edit_exercise(self, exercise: Exercise) -> None:
        ...
        new_title = self.prompt_input(f"Title [{exercise.title}]: ", 3, 4, exercise.title)

        # Use the multi-select for equipment editing
        equipment_options = ["none", "barbell", "dumbbell", "kettlebell", "machine", "bodyweight", "bands", "other"]
        initial_selection = [i for i, item in enumerate(equipment_options) if item in exercise.equipment]
        new_equipment_list = self._multi_select_list(equipment_options, "Select Equipment", initial_selection)
        ...
        while True:
            ...
            for idx, ex in enumerate(exercises):
                ...
                if idx == cursor:
                    ...
                    self.stdscr.addstr(3+idx, 2, line[:max_x-4])
            self.show_footer("↑/↓: Move | Enter: Delete | Esc: Back", 3)
            self.stdscr.refresh()
            k = self.stdscr.getch()
            if k in (curses.KEY_UP, ord('k')) and cursor > 0:
                cursor -= 1
    ...
    def view_history(self) -> None:
        ...
        while True:
            ...
            for idx, session in enumerate(filtered_sessions[:list_h]):
                y = 3 + idx
                date_str = f"{session.date!s:<{col_widths[0]}}"
...
# Define a main entry point to create a standalone workout logger
def run_tui(workout_log: DataManager = None) -> None:
    """Run the text-based user interface for the workout logger."""
    
    def main_tui(stdscr):
        ui = UIManager(stdscr, workout_log)
        ui.main_menu()
...
def main() -> None:
    ...
    # Run the TUI
    try:
        curses.wrapper(lambda stdscr: UIManager(stdscr, dm).main_menu())
    except Exception as e:
        logger.exception("Critical error in Ultimate Workout Logger")
